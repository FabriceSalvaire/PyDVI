...
---------------------------------
 Font Files Types and Extensions
---------------------------------

LaTeX uses the following types of font files:

* ``.fd`` - font definition: define a family of fonts,
* ``.mf`` - Metafont file: define the font shape,
* ``.pk`` - packet font: device dependent bitmap font,
* ``.pl`` - property list: human readable version of a TeX font metric file,
* ``.tfm`` - TeX font metric: describe the font metric and is completely
...
A Packed Font file can be generated from its Metafont source file using the command
...
Moreover the required file can be generated on the fly using the command :command:`kpsewhich` and
the option ``-mktex``::
